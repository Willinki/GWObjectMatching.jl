...
$$\operatorname{GW}(C,\mu;C^*,\mu^*) = \min_T  \sum_{\operatorname{i,j,k,l}} L(C_\operatorname{ik},C^*_\operatorname{jl})T_\operatorname{ij}T_\operatorname{kl}, $$

where $T \in \Gamma(\mu,\mu^*)$, i.e. $\sum_{j} T_{ij} = \mu_i$ and $\sum_i T_{ij} = \mu^*_j$. 
